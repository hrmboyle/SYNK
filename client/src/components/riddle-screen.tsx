            ...
            >
              {answers.map((answer, index) => (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  // Added text-center to center the text within the button
                  // Changed text-left to text-center
                  className='answer-button p-4 rounded-xl text-center transition-all'
                  onClick={() => onSelectAnswer(answer)}
                >
                  {/* Removed the A: B: prefix span */}
                  {/* Changed text color to text-mystical-pearl for white/light text */}
                  <span className='ml-0 text-mystical-pearl'>{answer}</span>
                </motion.button>
              ))}
            </motion.div>
          </div>
